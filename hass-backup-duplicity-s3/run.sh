...
if [[ ${RESTORE} == "true" ]]; then
    ...
    if [[ -n ${DAY_BEFORE_REMOVING_OLD_BACKUP} ]]; then
        ...
        duplicity --force --no-encryption --allow-source-mismatch --s3-endpoint-url "${ENDPOINT_URL}" --s3-region-name "${REGION}" --file-prefix-manifest manifest- remove-older-than ${DAY_BEFORE_REMOVING_OLD_BACKUP} "${BUCKET_NAME}"
    fi
fi